...
在引入 `categories` 字段后，脚本支持多级分类路径以及 `posts/` 下多层目录文章创建与识别验证。

### 1）创建多层目录文章

```bash
pnpm new-post "多级分类验证" --folder "示例/多级分类验证"
```
...
```bash
# 使用斜杠分隔（推荐）
pnpm new-post "多级分类验证" --field "categories=系统编程/操作系统/内存管理"
...
若 `astro check` 无 collection/schema 报错，说明多层目录与多级分类均可被正确识别。
