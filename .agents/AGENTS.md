...
### Q: 如何链接 reference docs？

**A**: reference docs 的实际 URL 以构建生成的 slug 为准。若路径中包含空格或英文大写，Astro 可能会规范化为小写连字符；不要直接在 Markdown 链接中使用带空格的裸路径，建议先通过 `pnpm build` 或构建日志确认实际路由。
